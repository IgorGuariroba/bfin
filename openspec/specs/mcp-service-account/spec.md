...
Define o modelo de identidade da service account que autoriza o servidor MCP:
token OIDC validado no bootstrap com audiência distinta da API HTTP, extração de
escopos `resource:action` para controle fino por tool, resolução do `userId` real
da base ao qual as writes serão atribuídas, e propagação de `meta.requestedBy`
como contexto **somente de auditoria** — nunca de autorização.
