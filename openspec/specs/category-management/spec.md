...
Define a capability de categorias do sistema (compartilhadas entre todas as contas) utilizadas para classificar movimentações e dívidas. Categorias possuem um `tipo` (`receita`, `despesa` ou `divida`) alimentado pela tabela `tipo_categorias`. Apenas usuários administradores podem criar, atualizar e deletar categorias; qualquer usuário autenticado pode listar.
