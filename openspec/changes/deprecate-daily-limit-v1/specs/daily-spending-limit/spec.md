...
> **DEPRECATED (sunset `{{DAILY_LIMIT_V1_SUNSET}}`):** este endpoint está marcado para remoção. Consumidores MUST migrar para `GET /contas/{contaId}/limite-diario-v2`. Durante o sunset, o comportamento funcional permanece inalterado; o sistema apenas sinaliza a deprecação via headers.
...
- `Sunset: {{DAILY_LIMIT_V1_SUNSET}}` (RFC 8594, formato HTTP-date)
...
- **THEN** a resposta inclui `Deprecation: true`, `Sunset: {{DAILY_LIMIT_V1_SUNSET}}` e `Link: </contas/{contaId}/limite-diario-v2>; rel="successor-version"`

### Requirement: Cálculo do saldo disponível

O sistema MUST preservar o cálculo v1 descrito abaixo durante o sunset para compatibilidade com consumidores que ainda usam a rota `/limite-diario`. **DEPRECATED (sunset `{{DAILY_LIMIT_V1_SUNSET}}`):** este cálculo permanece apenas como suporte à rota v1; a v2 (`/limite-diario-v2`) usa regra diferente e não depende deste requirement.
...
O sistema MUST preservar a divisão por `dias_restantes_no_mes` durante o sunset. **DEPRECATED (sunset `{{DAILY_LIMIT_V1_SUNSET}}`):** esta regra permanece apenas para a v1; a v2 usa `max(0, saldo_atual) / 30` com janela móvel de 30 dias.
...
O sistema MUST continuar garantindo a independência descrita abaixo durante o sunset. **DEPRECATED (sunset `{{DAILY_LIMIT_V1_SUNSET}}`):** requirement mantido para a v1; a v2 possui requirement próprio e mais amplo (`Independência de reserva e projeção (v2)`).
...
O sistema MUST marcar as tools MCP `daily-limit_get` e `daily-limit_set` como deprecated, prefixando o campo `description` com `[DEPRECATED — use <substituto>; sunset {{DAILY_LIMIT_V1_SUNSET}}]`:
...
- **THEN** a `description` de `daily-limit_get` começa com `[DEPRECATED — use daily-limit_v2_get; sunset {{DAILY_LIMIT_V1_SUNSET}}]`

#### Scenario: description da tool daily-limit_set sinaliza deprecação

- **WHEN** um cliente MCP lista as tools disponíveis
- **THEN** a `description` de `daily-limit_set` começa com `[DEPRECATED — use goals_create/goals_update; sunset {{DAILY_LIMIT_V1_SUNSET}}]`
